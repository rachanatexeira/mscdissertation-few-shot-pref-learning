...
import numpy as np
from gymnasium.wrappers import TransformObservation, FlattenObservation, RecordVideo
from gymnasium.spaces import Box



def get_env(env_name, env_kwargs, wrapper, wrapper_kwargs):
    # env = gym.make(env_name, **env_kwargs)
    env = gym.make(env_name, render_mode="rgb_array", **env_kwargs)
    env = RecordVideo(env, video_folder="./videos/sac_training", episode_trigger=lambda x: x % 10 == 0)


    from research.envs.reward_scale_wrapper import RewardScaleWrapper

    # ✅ Remove the SafeResetWrapper
    env = RewardScaleWrapper(env, scale=1.0)

    if not isinstance(env.observation_space, Box):
        env = FlattenObservation(env)
        
    # print(env.observation_space.dtype)
    # Forcefully convert observation space to float32 and recreate Box
    old_space = env.observation_space
    env.observation_space = Box(
        low=np.full(old_space.shape, -np.inf, dtype=np.float32),
        high=np.full(old_space.shape, np.inf, dtype=np.float32),
        dtype=np.float32
    )
    
    # print(env.observation_space.dtype)

    # ✅ Normalize observations for SAC stability
    def normalize_obs(obs):
        obs = obs.astype(np.float32)
        return np.clip(obs / 5.0, -1.0, 1.0)

    env = TransformObservation(env, normalize_obs, observation_space=env.observation_space)
    return env
...
#     if wrapper is not None:
#         if wrapper == "FlattenObservation":
#             from gymnasium.wrappers import FlattenObservation
#             env = FlattenObservation(env, **wrapper_kwargs)
#         else:
#             raise ValueError(f"Unsupported wrapper: {wrapper}")
    
#     return env

#     # Combine obs and goal into flat array
#     env = gym.wrappers.FlattenObservation(
#         gym.wrappers.FilterObservation(env, ["observation", "desired_goal"])
#     )
...
#                 low=original_space.low.astype(np.float32),
#                 high=original_space.high.astype(np.float32),
#                 dtype=np.float32,
#             )
#         )

#     print("🧠 Final Observation space:", env.observation_space)
#     return env
...
def train(config: Config, path: str, device: Union[str, torch.device] = "auto") -> Algorithm:
    ...
    import time  # add this at the top if it's not already
    ...
    end_time = time.time()
    print(f"[research] Training time: {end_time - start_time:.2f} seconds")
