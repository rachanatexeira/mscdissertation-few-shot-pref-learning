...
class ReplayBuffer(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        # NEW: assign directly (no kwargs.get)
    ...
    def _alloc(self):
        ...
        if self.capacity is not None:
            ...
            for data in self.preload():
                ...
                # print(f"[DEBUG] Added to buffer: obs={type(obs)}, action={action}, reward={reward}")
        else:
            self._capacity = None
    ...
    def add(
        ...
        **kwargs,
        ...
        # --- sanitize (keep your logic) ---
        if isinstance(obs, (list, tuple)):
            obs = next((o for o in obs if not isinstance(o, dict)), obs[0])
        if isinstance(action, (list, tuple)):
            action = next((a for a in action if not isinstance(a, dict)), action[0])

        # --- original replay-buffer push ---
        ...
        # --- episode bookkeeping + save-to-disk (put this back) ---
        if not hasattr(self, "current_ep"):
            self.current_ep = dict()

        is_list = isinstance(reward, (list, np.ndarray))
        ...
        #     # ensure a storage path exists; set this once in __init__ or via dataset_kwargs
        #     os.makedirs(self.storage_path, exist_ok=True)
        ...
        if is_done and self.save_episodes:
            ep_idx = self.num_episodes
            ep_len = len(self.current_ep["reward"])
            # Ensure all per-key lists align
            add_dummy_transition(self.current_ep, ep_len)
            self.num_episodes += 1

            if not self.storage_path:
                print("[replay] WARNING: save_episodes=True but storage_path is not set; skipping save.")
            else:
                os.makedirs(self.storage_path, exist_ok=True)
                ts = datetime.datetime.now().strftime("%Y%m%dT%H%M%S")
                ep_filename = f"{ts}_{ep_idx}_{ep_len}.npz"
                out_path = os.path.join(self.storage_path, ep_filename)
                save_data(self.current_ep, out_path)
                # print(f"[replay] Saved episode #{ep_idx} (len={ep_len}) to {out_path}")
        ...
        if hasattr(self, "storage_path"):
            try:
                ...
                try:
                    ...
                    print(f"⚠️ Could not remove directory {self.storage_path}: {e}")
            except Exception as e:
                print(f"⚠️ ReplayBuffer cleanup failed: {e}")
    ...
    def sample(self, batch_size: Optional[int] = None, stack: int = 1, pad: int = 0) -> Dict:
        ...
        # print(f"[DEBUG] Sampled batch keys: {list(batch.keys())}")
        # print(f"[DEBUG] obs batch shape: {batch['obs'].shape if 'obs' in batch else None}")
        # print(f"[DEBUG] action batch shape: {batch['action'].shape if 'action' in batch else None}")
        # print(f"[DEBUG] reward batch shape: {batch['reward'].shape if 'reward' in batch else None}")
