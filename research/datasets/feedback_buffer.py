...
class FeedbackLabelDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        for i in range(len(idxs)):
            sample = self._sample([idxs[i]])
            yield sample  # Only one item at a time
...
class MultiTaskOracleFeedbackDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(paths, str):
            ...
            paths = [os.path.join(paths, p) for p in os.listdir(paths) if os.path.isdir(os.path.join(paths, p))]
    ...
    # def __iter__(self):
    #     try:
    #         for task_idx, (task_name, dataset) in enumerate(self._datasets.items()):
    #             # print(f"🎯 Dataset {task_idx} ({task_name}) type: {type(dataset)}")
    #             # print("🔍 Full dataset content:", dataset)
    #             for i, sample in enumerate(dataset):
    #                 # print(f"✅ Sample {i} from dataset {task_idx}")
    #                 yield sample
    #                 if i == 1 and task_idx == 0:
    #                     break
    #     except Exception as e:
    #         print("Error while iterating dataset:", e)
    #         raise
    ...
    def __iter__(self):
        ...
        while True:
            for task, iterator in iterators.items():
                ...
                try:
                    for _ in range(64):  # ← match your batch size (num_support + num_query)
                        sample = next(iterator)
                        for k in batch:
                            batch[k].append(sample[k])
                except StopIteration:
                    # One of the tasks ran out of data — consider this one epoch
                    return

                # Convert list of samples to np arrays
                for k in batch:
                    batch[k] = np.stack(batch[k], axis=0)  # (64, ...)
