...
class Algorithm(ABC):
    ...
    def train(
        ...
        #     print("IterableDataset detected.")
        # else:
        #     print("Dataset is indexable.")
        ...
        with tqdm(total=total_steps, desc="Training") as pbar:
            while current_step <= total_steps:
                # print("Waiting to receive batch from dataloader...")

                for batch in dataloader:
                    # print("✔️ Got batch of type:", type(batch))
    ...
    def _predict(self, batch: Any, **kwargs) -> Any:
        ...
        with torch.no_grad():
            ...
            if isinstance(pred, torch.Tensor):
                ...
                pred = np.array([p.detach().cpu().numpy() if isinstance(p, torch.Tensor) else p for p in pred])
            elif not isinstance(pred, np.ndarray):
                raise ValueError(f"Unsupported type in predict: {type(pred)}")
