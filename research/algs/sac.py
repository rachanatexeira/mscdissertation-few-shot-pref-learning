...
class SAC(Algorithm):
    ...
    def _update_critic(self, batch: Dict) -> Dict:
        with torch.no_grad():
            ...
            next_action = next_action.clamp(-1.0 + eps, 1.0 - eps)
    ...
    def _step_env(self) -> Dict:
        ...
        # print(f"Step reward: {reward}, Distance: {np.linalg.norm(next_obs['achieved_goal'] - next_obs['desired_goal'])}")
    ...
    def _predict(self, batch: Any, sample: bool = False) -> torch.Tensor:
        ...
        epsilon = 1e-6
        self.action_range = [
            float(self.action_space.low.min()) + epsilon,
            float(self.action_space.high.max()) - epsilon,
        ]

        # 🔹 Handle Gymnasium dict observations (FetchReach, etc.)
        if isinstance(obs, dict):
            # Combine robot state + desired goal into a single flat array
            obs = np.concatenate([obs["observation"], obs["desired_goal"]], axis=-1)
        elif isinstance(obs, list) or isinstance(obs, tuple):
